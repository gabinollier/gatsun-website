...
import { PricingService } from '@/lib/types/pricing.d';
...
export default function PricingCards({ services, footnotes, isPreview = false }: PricingCardsProps) {
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {services.map((service, index) => (
          <a 
            ...
            className={`${isPreview ? '' : 'intersect-once md:intersect:motion-preset-fade-md motion-duration-1000'} motion-delay-${index * 100} relative bg-slate-800 rounded-xl shadow-xl ring hover:ring-2 ring-slate-600 flex flex-col transition duration-300 ease-in-out transform hover:-translate-y-2 hover:shadow-orange-950 hover:ring-orange-500 cursor-pointer`} 
          >
            {service.popular ? (
